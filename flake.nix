{
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        # Define the bootDebugISO configuration
        # Build with: nix build .#bootDebugISO.config.system.build.isoImage
        packages.default = (nixpkgs.lib.nixosSystem {
          ...
          modules = [
            ...
            ({ config, pkgs, modulesPath, ... }: {
              ...
              # Show helpful message on ISO boot
              environment.motd = ''
                ╔════════════════════════════════════════════════╗
                ║  NixOS Minimal ISO with Debug Logging Enabled  ║
                ║                                                ║
                ║  Boot log capture:                             ║
                ║    journalctl --boot --all                     ║
                ║                                                ║
                ║  Export to file:                               ║
                ║    journalctl --boot --all > /tmp/boot.log     ║
                ╚════════════════════════════════════════════════╝
              '';
            })
          ];
